...
| 技能 | 职责 | 触发关键词 |
|------|------|------------|
| **mybatis-plus-best-practices** | MyBatis-Plus CRUD、Wrapper、分页 | MyBatis-Plus、MP、Wrapper |
| **redis-cache-guide** | Redis 缓存策略、分布式锁 | Redis、缓存、分布式锁 |
...
| 技能 | 职责 | 触发关键词 |
|------|------|------------|
| **ddd-domain-driven-design** | DDD 战略/战术设计 | DDD、领域、聚合、限界上下文 |
| **technical-writing** | 技术文档写作 | 文档、README、API 文档 |
| **code-review** | 代码审查规范 | 代码审查、CR、Review |
...
| 技能 | 职责 | 触发关键词 |
|------|------|------------|
| **tenant-management** | 多租户管理、TenantEntity | 多租户、SaaS、租户隔离 |
| **redis-cache** | Redis 缓存基础 | Redis、缓存配置 |
...
通过 `skill-forced-eval.js` 钩子自动匹配：

```javascript
// 用户输入检测
if (input.includes('CRUD') || input.includes('增删改查')) {
  // 触发 crud-development 技能
}
```
...
```
请使用 vue-best-practices 技能开发用户列表页面
```
